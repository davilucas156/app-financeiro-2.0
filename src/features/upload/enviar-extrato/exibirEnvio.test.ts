...
const linha = (
  parcial: Partial<LinhaDeImportacao> = {},
): LinhaDeImportacao => ({
  id: "3f2b",
...
describe("enviadoEmPtBr", () => {
  it("mostra o horário de São Paulo, não o do servidor", () => {
    ...
    expect(enviadoEmPtBr(new Date("2026-08-19T00:12:00Z"))).toBe(
      "18/08 às 21h",
    );
  });

  it("não muda de dia quando o instante já é o mesmo dia nos dois fusos", () => {
    expect(enviadoEmPtBr(new Date("2026-08-18T14:00:00Z"))).toBe(
      "18/08 às 11h",
    );
  });
  ...
  it("meia-noite em São Paulo sai como 00h, nunca 24h", () => {
    ...
    expect(enviadoEmPtBr(new Date("2026-06-10T03:00:00Z"))).toBe(
      "10/06 às 00h",
    );
  });

  it("dia e mês vêm com dois dígitos", () => {
    expect(enviadoEmPtBr(new Date("2026-03-05T15:00:00Z"))).toBe(
      "05/03 às 12h",
    );
  });
});
...
describe("paraEnvioExibido", () => {
  it("traduz a origem para a palavra que aparece na tela", () => {
    ...
    expect(
      paraEnvioExibido(linha({ origem: "csv_cartao" })).rotuloDeOrigem,
    ).toBe("cartão");
  });
  ...
  it("um envio sem nenhuma linha válida aparece com zero", () => {
    ...
    // desfazer.
    expect(
      paraEnvioExibido(linha({ lancamentosImportados: 0 })).lancamentos,
    ).toBe(0);
  });
});
